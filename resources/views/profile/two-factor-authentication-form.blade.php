<x-action-section>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('قم بإنهاء تفعيل المصادقة الثنائية.') }}
                @else
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('عند تمكين المصادقة الثنائية، سيُطلب منك إدخال رمز عشوائي آمن أثناء المصادقة. يمكنك استرداد هذا الرمز من تطبيق Google Authenticator على هاتفك.') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        ...
                        @else
                            {{ __('تم الآن تمكين المصادقة الثنائية. قم بمسح رمز الاستجابة السريعة التالي باستخدام تطبيق المصادقة على هاتفك أو أدخل مفتاح الإعداد.') }}
                        @endif
                    </p>
                </div>
