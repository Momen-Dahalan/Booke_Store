    ...
    public function addToCart(Request $request)
    {
        ...
        if (auth()->user()->booksInCart->contains($book)) {
            ...
            if ($newQuantity > $book->number_of_copies) {
                session()->flash('warning_message', 'لم تتم اضافة الكتاب لقد تجاوزت عدد النسخ الموجودة لدينا , اقصى عدد موجود لدينا هو  ' . ($book->number_of_copies - auth()->user()->booksInCart()->where('book_id', $book->id)->pivot->number_of_copies) . ' كتاب');
                return redirect()->back();
            }else{
                auth()->user()->booksInCart()->updateExistingPivot($book->id , ['number_of_copies' => $newQuantity]);
            }
        }else{
            auth()->user()->booksInCart()->attach($request->bookId , ['number_of_copies' => $request->quantity]);
        }
